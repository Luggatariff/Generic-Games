...
Carlo::Carlo(string name, unsigned int team_id, unsigned int level, unsigned int max_free_choices, bool display_messages, bool display_tree){
	ia_name = name;
	ia_team = team_id;
...
Coordinates Carlo::play(Game * game , vector<Coordinates> limit_choices){
	...
	map<Coordinates, CarloTree *> choices;
	...
	if (limit_choices.size() > 1){
		map<Coordinates, CarloTree *>::iterator choices_it = choices.begin();
	...
	if (choices.empty()){
		...
		for (vector<Coordinates>::iterator move = playable_moves.begin(); move != playable_moves.end(); move++){
			...
			new_game->play(*move);
			choices.insert(pair<Coordinates, CarloTree *>((*move), new CarloTree(new_game, this)));
		}
	}
	...
	while(choices.size() > 1 && level <= max_level){
		...
		vector<unsigned int> random_indexes = CarloTree::pick_random_indexes(choices.size(), choices.size());

		vector<unsigned int>::iterator random_indexes_iterator = random_indexes.begin();
		map<Coordinates, CarloTree *>::iterator choices_iterator = choices.begin();
		...
		CarloScore * best_choice_score = choices_iterator->second->getScore();
		...
		if ( (best_choice_score->value() < victory_score && level <= max_level) || (best_choice_score->value() < 0 && level == max_level) ) {
			for(random_indexes_iterator++; random_indexes_iterator != random_indexes.end(); random_indexes_iterator++){
				...
				CarloScore * res_score = choices_iterator->second->getScore();
		...
		while (choices_iterator != choices.end()){
			CarloScore * choice_score = choices_iterator->second->getScore();
	...
	map<Coordinates, CarloTree *>::iterator choice = choices.begin();
	if (choices.size() > 1){
	...
	for (map<Coordinates, CarloTree *>::iterator choices_iterator = choices.begin(); choices_iterator != choices.end(); choices_iterator++){
		delete choices_iterator->second;
	}
